...
class Login extends CI_Controller {
    ...
    public function simple() {
        ...
        $this->load->view('login_simple');
    }
    
    public function authenticate() {
        // Método de login que redirige directamente desde el servidor
        $usuario = $this->input->post('log', TRUE);
        $password = $this->input->post('pass', TRUE);
        
        // Validaciones básicas
        if (empty($usuario) || empty($password)) {
            $this->session->set_flashdata('error', 'Usuario y contraseña son requeridos');
        ...
        if ($query->num_rows() == 1) {
            ...
            $this->session->set_userdata($session_data);
            $this->session->set_flashdata('success', '¡Bienvenido ' . $user->nombre . '!');
            
            // Redirigir al home original
            redirect('home');
