...
class mArea extends BaseModel{
    ...
    public function doCreateObject(array $fields){
        $dmnArea = new dmnArea($fields['id']);
        $dmnArea->setNombre($fields['nombre']);        
        
        return $dmnArea;
    }

    public function create(dmnArea $dmnArea){
        try{
            $this->db->trans_begin();
            $this->doCreate($dmnArea);
    ...
    private function doCreate(dmnArea $dmnArea){
        
//        $fields['id'] = $dmnArea->getId();
        $fields['nombre'] = $dmnArea->getNombre();        
    ...
    public function update(dmnArea $dmnArea){
        try{
            $this->db->trans_begin();
            $this->doUpdate($dmnArea);
            $this->db->trans_commit();
        }catch(Exception $e){
            $this->db->trans_rollback();
            throw new Exception($e->getMessage(), $e->getCode());
    ...
    private function doUpdate(dmnArea $dmnArea){

//        $fields['id'] = $dmnArea->getId();
        $fields['nombre'] = $dmnArea->getNombre();
               
        $this->db->set($fields);
        $this->db->where(array('ID' => $dmnArea->getId()));
    ...
    public function delete(dmnArea $dmnArea){
        try{
            $this->db->trans_begin();
            $this->doDelete($dmnArea);
            $this->db->trans_commit();
        }catch(Exception $e){
            $this->db->trans_rollback();
            throw new Exception($e->getMessage(), $e->getCode());
    ...
    private function doDelete(dmnArea $dmnArea){
        $this->db->where(array('id' => $dmnArea->getId()));
