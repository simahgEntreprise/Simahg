<?php
require_once APPPATH.'models/BaseModel.php';
require_once DOMAIN_PATH.'configuracion/dmnPerfil.php';
class mPerfil extends BaseModel{
   // protected  $fields = 'id,nombre,flgAccesoMovil';
    protected  $fields = 'id,nombre,flgaccesomovil';

    protected $table ='t_perfil';
    public function  __construct() {
        parent::__construct();
    }
    public function find(array $constraints){
        $this->db->select($this->fields);
        if(isset($constraints['id'])){
            $this->db->where('id',$constraints['id']);
        }elseif(isset ($constraints['nombre'])) {
            $this->db->where('nombre',$constraints['nombre']);            
        }  else {
            $this->session->set_flashdata('perfil_incorrecto','Los datos introducidos son incorrectos');
        }
        $res = $this->db->get($this->table);        
        return $this->createObject($res);
    }

    public function finder(array $constraints, $begin = 0, $end = 0){
        $this->load->database();
        $this->db->select($this->fields);

        if(isset($constraints['nombre']) && strlen(trim($constraints['nombre'])) > 0){
            $this->db->like('nombre',$constraints['nombre']);
        }
        if(isset($constraints['id']) && strlen(trim($constraints['id'])) > 0){
            $this->db->where('id',$constraints['id']);
        }
//        print_r($this->db->last_query());
        
        $res = $this->db->get($this->table);
        if($res == null){
            throw new Exception('Error en MapperPerfil '.$this->db->last_query(),-1);
        }
        return $this->paginate($res);
    }

    public function doCreateObject(array $fields){
        $dmnPerfil = new dmnPerfil($fields['id']);
        $dmnPerfil->setNombre($fields['nombre']);
        $dmnPerfil->setFlgAccesoMovil($fields['flgaccesomovil']);
        return $dmnPerfil;
    }

    public function create(dmnPerfil $dmnPerfil){
        try{
            $this->db->trans_begin();
            $this->doCreate($dmnPerfil);
            $id = $this->setIdPerfil();
//            $dmnPerfil->setId($id) ;
            $this->db->trans_commit();
        }catch(Exception $e){
            $this->db->trans_rollback();
            throw new Exception($e->getMessage(), $e->getCode());
        }
    }

    private function doCreate(dmnPerfil $dmnPerfil){
        
//        $fields['id'] = $dmnPerfil->getId();
        $fields['nombre'] = $dmnPerfil->getNombre();
        $fields['flgAccesoMovil']= $dmnPerfil->getFlgAccesoMovil();
        
        $this->db->set($fields);
        $this->db->insert($this->table);

        if($this->db->trans_status() === false){
            throw new Exception("Hubo un problema al Registrar perfil, revisar los datos Ingresados: ". $this->db->last_query(), -1);
        }
    }

    public function update(dmnPerfil $dmnUsuario){
        try{
            $this->db->trans_begin();
            $this->doUpdate($dmnUsuario);
            $this->db->trans_commit();
        }catch(Exception $e){
            $this->db->trans_rollback();
            throw new Exception($e->getMessage(), $e->getCode());
        }
    }

    private function doUpdate(dmnPerfil $dmnPerfil){

//        $fields['id'] = $dmnUsuario->getId();
        $fields['nombre'] = $dmnPerfil->getNombre();
        $fields['flgAccesoMovil']= $dmnPerfil->getFlgAccesoMovil();

        $this->db->set($fields);
        $this->db->where(array('ID' => $dmnPerfil->getId()));
        $this->db->update($this->table);

        if($this->db->trans_status() === false){
            throw new Exception("Hubo un problema al Modificar Usuario, revisar los datos Ingresados: ". $this->db->last_query(), -1);
        }
    }

    public function delete(dmnPerfil $dmnPerfil){
        try{
            $this->db->trans_begin();
            $this->doDelete($dmnPerfil);
            $this->db->trans_commit();
        }catch(Exception $e){
            $this->db->trans_rollback();
            throw new Exception($e->getMessage(), $e->getCode());
        }
    }

    private function doDelete(dmnPerfil $dmnPerfil){
        $this->db->where(array('id' => $dmnPerfil->getId()));
        $this->db->delete($this->table);

        if($this->db->affected_rows()==0){
            throw new Exception("Hubo un problema al Eliminar perfil, revisar los datos Ingresados: ". $this->db->last_query(), -1);
        }
        if($this->db->trans_status() === false){
            throw new Exception("Hubo un problema al Eliminar perfil, revisar los datos Ingresados: ". $this->db->last_query(), -1);
        }
    }    
    private function setIdPerfil(){
        try{            
            $res = $this->db->query("SELECT MAX(ID) as id FROM T_PERFIL");
            return $res->result_array()[0]['id'];
        }catch(Exception $e){

        }
        
    }
    
    
}

