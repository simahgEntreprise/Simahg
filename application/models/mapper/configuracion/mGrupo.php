...
class mGrupo extends BaseModel{
    ...
    public function finder(array $constraints){
        ...
        $res = $this->db->get($this->table);
        if($res == null){
            throw new Exception('Error en MapperArea'.$this->db->last_query(),-1);
        }
        return $this->paginate($res);
    }

    public function doCreateObject(array $fields){
        $dmnGrupo = new dmnGrupo($fields['id']);
        $dmnGrupo->setNombre($fields['nombre']);        
        
        return $dmnGrupo;
    }

    public function create(dmnGrupo $dmnGrupo){
        try{
            $this->db->trans_begin();
            $this->doCreate($dmnGrupo);
    ...
    private function doCreate(dmnGrupo $dmnGrupo){
        
//        $fields['id'] = $dmnGrupo->getId();
        $fields['nombre'] = $dmnGrupo->getNombre();        
    ...
    public function update(dmnGrupo $dmnGrupo){
        try{
            $this->db->trans_begin();
            $this->doUpdate($dmnGrupo);
            $this->db->trans_commit();
        }catch(Exception $e){
            $this->db->trans_rollback();
            throw new Exception($e->getMessage(), $e->getCode());
    ...
    private function doUpdate(dmnGrupo $dmnGrupo){

//        $fields['id'] = $dmnGrupo->getId();
        $fields['nombre'] = $dmnGrupo->getNombre();
               
        $this->db->set($fields);
        $this->db->where(array('ID' => $dmnGrupo->getId()));
    ...
    public function delete(dmnGrupo $dmnGrupo){
        try{
            $this->db->trans_begin();
            $this->doDelete($dmnGrupo);
            $this->db->trans_commit();
        }catch(Exception $e){
            $this->db->trans_rollback();
            throw new Exception($e->getMessage(), $e->getCode());
    ...
    private function doDelete(dmnGrupo $dmnGrupo){
        $this->db->where(array('id' => $dmnGrupo->getId()));
