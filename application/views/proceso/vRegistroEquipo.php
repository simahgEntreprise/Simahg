...
<div class="modal fade" id="myModal" tabindex="-1" role="dialog" aria-labelledby="myModalLabel">
    ...
    <div class="modal-header">
        ...
        <h4 class="modal-title" id="myModalLabel">Registro de Equipos</h4>
    </div>
	...
	<form role="form" id="frmItem">            
            ...
            <div class="form-group">
                ...
                <input class="form-control" id="asignado">
            </div>
            <div class="form-group">
                <label>Fecha de Solicitud</label>
                <input class="form-control" type="date" id="fecSol">
            </div>
            <div class="form-group">
                <label>Fecha de Retorno</label>
                <input class="form-control" type="date" id="fecRet">
            </div>
      ...
      <div class="modal-footer">
        ...
        <button type="button" class="btn btn-primary" onclick="ctrItem();">Guardar</button>        
      </div>
    </div>
  </div>
</div>
    ...
    $(document).ready(function() {
        var source =
            {                               
                url: base_url+"proceso/equipo/getList",
                datatype: "json",
                root: 'data',
                datafields: [
                    { name: 'id' },                    
                    { name: "equipo", map: "equipo>nombre"},
                    { name: "responsable" },
                    { name: "fecentrega" },
		...
		height: "100%",                
                ...
                columns: [
                  ...
                  { text: 'Elemento', dataField: 'equipo', width: '30%' },
                  { text: 'Responsable', dataField: 'responsable', width: '30%' },
                  { text: 'Fecha', dataField: 'fecentrega', width: '30%' },                  
                  { text: 'Accion', dataField: 'accion', width: '10%', cellsrenderer: function (row, column, value) {
                        return '<button type="button" class="btn btn-info btn-circle" data-toggle="modal" data-target="#myModal" onclick="editItem('+value+')"><i class="fa fa-edit"></i></button>';
                    }
                  }
                ]
            });                                              
        ...
        $.ajax({
            type: "POST",
            datatype:'json',                
            url:base_url+"configuracion/equipoConf/getList",
...
//     $("#cboElement").on('onchange', function () {
//                
//     });
    ...
    $('#tipo').val ('U');
         $.ajax({
            ...
            success : function(text){
                ...
                $('#cboElement').val(datos.equipo.id);
                $('#idItem').val(datos.id);
                $('#respon').val(datos.responsable);
                $('#fecSol').val(datos.fecsolicitud);
                $('#fecRet').val(datos.fecentrega);
    ...
    function ctrItem(){
            var tipo = $('#tipo').val();
            var equip = $('#cboElement').val();
            var resp = $('#respon').val();
            var fecSol = $('#fecSol').val();
            var fecEnt = $('#fecRet').val();
            var idItem = $('#idItem').val();
            var asign = $('#asignado').val();
            
            
            if (equip === 0) {
                $.msgBox({
                    title:"Atenci&#243;n",
                    content:"Ingrese el equipo a registrar",
                    type:"info"
                });
                return false;
            }
            ...
            if (resp.trim()==="") {
                $.msgBox({
                    title:"Atenci&#243;n",
                    content:"Ingrese un responsable",
                    type:"info"
                });
                return false;
            }
            if (fecSol.trim()==="") {
                $.msgBox({
                    title:"Atenci&#243;n",
                    content:"Ingrese la fecha de Solicitud",
                    type:"info"
                });
                return false;
            }
            ...
            if (fecEnt.trim()==="") {
                $.msgBox({
                    title:"Atenci&#243;n",
                    content:"Ingrese la fecha de Retorno",
                    type:"info"
                });
                return false;
            }
            ...
            if (fecSol > fecEnt){
                $.msgBox({
                    title:"Atenci&#243;n",
                    content:"la fecha de retorno es menor a la fecha de solicitud",
                    type:"info"
                });
                return false;
            }
        ...
        jsShowWindowLoad('Espere');
                ...
                data.append('equipo',equip);
                data.append('respon',resp);
                data.append('asign', asign)
                data.append('fecSol',fecSol);
                data.append('fecEnt',fecEnt);
                ...
                cache:false,
                data: data,
                url:base_url+"proceso/equipo/create",
                success : function(text){
                    var data = $.parseJSON(text);                                        
                    if (data.success){
                        $('#btnCerrar').click();
