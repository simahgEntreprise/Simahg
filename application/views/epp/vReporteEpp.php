...
<head>
    ...
    <script type="text/javascript" src="<?php echo base_url();?>js/jqwidgets/jqxmenu.js"></script>
    <script type="text/javascript" src="<?php echo base_url();?>js/jqwidgets/jqxgrid.grouping.js"></script>
</head>
...
<body>
    ...
    <div id="wrapper">
        ...
        <div id="page-wrapper">
            <div class="row">
                <div class="col-lg-12">
                    <h1 class="page-header">Reporte de EPP's</h1>
                </div>
                <!-- /.col-lg-12 -->
            </div>
            <!-- /.row -->
    ...
    $(document).ready(function() {
        var source =
            {                               
                url: base_url+"epp/rptAsignacionEpp/getList",
                datatype: "json",
                root: 'data',
                datafields: [
                    { name: 'nombre' },                    
            ...
            $("#jqxgrid").on('pagechanged', function () {
             ...
             var groupsrenderer = function (text, group, expanded, data) {                 
                if (data.groupcolumn.datafield == 'viewLider' ) {
                    var number = dataAdapter.formatNumber(group, data.groupcolumn.cellsformat);
//                    var text = data.groupcolumn.text                     
var arr = group.split("-");
                    return '<div style="position: absolute;"><span>' + text + ' <button class="btn btn-primary btn-xs" type="button" onclick="generarep(' + arr[0] + ');">Generar Reporte</button></div>';
                }
                else {
                    var tx = data.groupcolumn.text                                         
                    
                    return '<div style="position: absolute;"><span>' + text + 'Total </div>';
                }
            }
		...
		height: "100%",                
                ...
                groupable : true,
                columns: [
                  { text: '', dataField: 'accion', width: '3%', cellsrenderer: function (row, column, value) {                        
                        var img='';
                        if (value==="3"){
                          img='<button type="button" class="btn btn-success btn-circle" data-toggle="modal" data-target="#myModal"></button>';
                        }else if(value==="2"){
                          img='<button type="button" data-toggle="modal" class="btn btn-warning btn-circle" data-target="#myModal"></button>';
                        }else{
                          img='<button type="button" class="btn btn-danger btn-circle" data-toggle="modal" data-target="#myModal"></button>';
                        }
                      return img;                        
                    }
                  },
                  { text: 'Nombre', dataField: 'nombre', width: '20%'},                  
                  { text: 'apellidos', dataField: 'apellidos', width: '20%' },
                  { text: 'Factor', dataField: 'factor', width: '10%' },
                  { text: 'Descripcion', dataField: 'descripcion', width: '20%' },
                  { text: 'Tipo', dataField: 'tipo', width: '15%' },                  
                  { text: 'Fecha Fin', dataField: 'fecfin', width: '12%' },                  
                  { text: 'Proyecto', dataField: 'proyecto', width: '12%',hidden: true },
                  { text: 'idLider', dataField: 'idLider', width: '12%',hidden: true },
                  { text: 'xLider', dataField: 'lider', width: '12%',hidden: true },
                  { text: 'Lider', dataField: 'viewLider', width: '12%',hidden: true },
                  { text: 'Responsable', dataField: 'responsable', width: '12%',hidden: true }                  
                ],
                groups: ['viewLider']
            });                                              
...
if(!id){
 $.msgBox({
                        title:"Atenci&#243;n",
                        content:"No se ha obtenido el codigo de lectura",
                        type:"info"
                    });  
return false;
}else{
...
return false;
        ...
        window.open("<?php echo base_url();?>index.php/reportes/reporteEpp/printReport?id="+id);
