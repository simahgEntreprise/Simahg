                ...
                <div class="col-lg-12">
                    <div class="panel panel-default">                                               
                        <div class="panel-body">                            
                            <p><button type="button" class="btn btn-primary" data-toggle="modal" data-target="#myModal" onclick="document.getElementById('frmItem').reset();$('#files').fileinput('reset'); document.getElementById('list').innerHTML = [''].join(''); $('#tipo').val ('C');">Nuevo</button>                               
                            </p>
...
<div class="modal fade" id="myModal" tabindex="-1" role="dialog" aria-labelledby="myModalLabel">
    ...
    <div class="modal-header">
        ...
        <h4 class="modal-title" id="myModalLabel">Registros de EPP's</h4>
    </div>
	...
	<form role="form" id="frmItem" enctype="multipart/form-data">            
            ...
            <div class="form-group">
                <label for="cboRiesgo">Riesgo</label>
                <select id="cboRiesgo" class="form-control">                                         
                </select>                                            
            </div>  
            <div class="form-group">
                <label for="cboTipoEpp">Tipo</label>
                <select id="cboTipoEpp" class="form-control">                                         
                </select>                                            
            </div>                  
            <div class="form-group">
                <label>Uso</label>
                <input class="form-control" id="txtUso" size="30" maxlength="30">
            </div>
            <div class="row">                
                <div class="col-md-6">
                    <div class="form-group">
                    <label for="cboVidaUtil">Vida Util</label>
                    <select id="cboVidaUtil" class="form-control">                                         
                ...
                <div class="col-md-6">
                    ...
                    <input class="form-control" id="txtCantidad" size="30" maxlength="30">
                    </div>
                </div>
            </div>
            <div class="form-group">                
            ...
            <div class="form-group">
                ...
                <input class="form-control" id="txtEspecificacion" size="50" maxlength="50" required="required">
            </div>            
            <div class="form-group">
                <label>Norma</label>
                <input class="form-control" id="txtNorma" size="50" maxlength="50" required="required">
            </div>                 
            <div class="form-group">
    ...
    $(document).ready(function() {                            
        var source =
            {                               
                url: base_url+"epp/registroEpp/getList",
                datatype: "json",
                root: 'data',
                datafields: [
                    { name: 'id' },
                    { name: 'riesgo', map:'riesgo>factor' },                   
                    { name: 'tipoEpp', map:'tipoEpp>nombre' },                   
                    { name: 'uso' },                   
                    { name: 'estado' },                   
		...
		height: "100%",                
                ...
                columns: [
                  ...
                  { text: 'Riesgo', dataField: 'riesgo', width: '30%' },
                  { text: 'Tipo', dataField: 'tipoEpp', width: '20%' },                  
                  { text: 'Uso', dataField: 'uso', width: '30%' },                  
        ...
        $.ajax({
            type: "POST",
            datatype:'json',                
            url:base_url+"epp/riesgo/getList",
            success : function(text){
                var data = $.parseJSON(text);         
                var valor = data.data;                            
                var option='<option value="0">--Seleccione--</option>'
                for(var i=0;i< valor.length;i++){
                    option=option+"<option value="+valor[i].id+">"+valor[i].factor+"</option>";
                }
                $('#cboRiesgo').html(option);                
            }
        });
        ...
        $.ajax({
            type: "POST",
            datatype:'json',                
            url:base_url+"epp/tipoEpp/getList",
            success : function(text){
                var data = $.parseJSON(text);         
                var valor = data.data;
                var option='<option value="0">--Seleccione--</option>'
    ...
    $('#id').val(id);
        $.ajax({
            ...
            success : function(text){
                ...
                $('#id').val(datos.id);                
                $('#cboRiesgo').val(datos.riesgo.id);
                $('#cboTipoEpp').val(datos.tipoEpp.id);
                $('#txtUso').val(datos.uso);
                $('#txtEspecificacion').val(datos.especificacion);
                $('#resp').val(datos.responsable);                
                $('#cboVidaUtil').val(datos.vidaUtil);
                $('#txtCantidad').val(datos.tiempoVidaUtil);
                $('#txtNorma').val(datos.norma);                
                $('#cboEstado').val(datos.estado);
                var arrImage = datos.imagen;
                
                var img ='<img src="'+  arrImage+' " class="file-preview-image">'
...
//                    })(f);
        ...
        function ctrItem(){            
            ...
            var tipo = $('#tipo').val();
            var file = $('#files').val();            
            var riesgo = $('#cboRiesgo').val();
            var tipoEpp = $('#cboTipoEpp').val();
            var uso =  $('#txtUso').val();
            var especificacion = $('#txtEspecificacion').val();            
            var vida =  $('#cboVidaUtil').val();
            var tvida = $('#txtCantidad').val();
            var norma = $('#txtNorma').val();            
            var est = $('#cboEstado').val();
                        
            if (riesgo==='0' || tipoEpp === '0' || uso.trim()==='' || especificacion.trim() === '' || vida==='0' || tvida.trim()==='' || norma.trim()==='' || est === '0') {
                $.msgBox({
                    title:"Atenci&#243;n",
                ...
                data.append('tipoEpp',tipoEpp);           
                data.append('uso',uso);           
                data.append('especificacion',especificacion);           
                data.append('vida',vida);           
                data.append('cant',tvida);           
                data.append('norma',norma);           
                ...
                cache:false,
                data: data,
                url:base_url+"epp/registroEpp/create",
                xhr: function() {
