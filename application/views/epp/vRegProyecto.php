...
<head>
    ...
    <script type="text/javascript" src="<?php echo base_url();?>js/jqwidgets/jqxmenu.js"></script>
    <script type="text/javascript" src="<?php echo base_url();?>js/jqwidgets/jqxcheckbox.js"></script>
                ...
                <div class="col-lg-12">
                    <div class="panel panel-default">                                               
                        <div class="panel-body">                            
                            <p><button type="button" class="btn btn-primary" data-toggle="modal" data-target="#myModal" onclick="document.getElementById('frmItem').reset();$('#files').fileinput('reset'); $('#tipo').val ('C'); loadTrab('');">Nuevo</button>                               
                            </p>
...
<div class="modal fade" id="myModal" tabindex="-1" role="dialog" aria-labelledby="myModalLabel">
    ...
    <div class="modal-header">
        ...
        <h4 class="modal-title" id="myModalLabel">Registro de Proyectos</h4>
    </div>
	...
	<form role="form" id="frmItem" enctype="multipart/form-data">            
            ...
            <div class="form-group">
                ...
                <input class="form-control" id="txtResponsable" size="50" maxlength="50" required="required">
            </div>  
            <div class="form-group">
                <label for="cboLider">Lider</label>
                <select id="cboLider" class="form-control">                                         
                </select>                                            
            </div>                              
            <div class="form-group">
            ...
            <div id='jqxWidget1' style="font-size: 13px; font-family: Verdana; float: left;">
                <div id="jqxgrid1" style="width: 500px"></div>                    
            </div>
            </div>
        </form>
    </div>
    ...
    $(document).ready(function() {                            
        var source =
            {                               
                url: base_url+"epp/regProyecto/getList",
                datatype: "json",
                root: 'data',
                datafields: [
                    { name: 'id' },
                    { name: 'nombre' },                   
                    { name: 'responsable', map:'responsable' },                                       
                    { name: 'estado' },                   
		...
		height: "100%",                
                ...
                columns: [
                  ...
                  { text: 'Proyecto', dataField: 'nombre', width: '40%' },       
                  { text: 'Responsable', dataField: 'responsable', width: '40%' },
        ...
        $.ajax({
            ...
            success : function(text){
                ...
                for(var i=0;i< valor.length;i++){
                    option=option+"<option value="+valor[i].id+">"+valor[i].nombre +" "+valor[i].apellidos+"</option>";
                }                
                $('#cboLider').html(option);                
            }
        });
    ...
    $('#id').val(id);
        $.ajax({
            ...
            success : function(text){
                ...
                $('#id').val(datos.id);                
                $('#cboLider').val(datos.lider.id);
                $('#txtNombre').val(datos.nombre);
                $('#txtResponsable').val(datos.responsable);                
    ...
    function deleteItem(id){
        ...
        function ctrItem(){            
            var id = $('#id').val();
            var tipo = $('#tipo').val();            
            var lider = $('#cboLider').val();            
            var resp =  $('#txtResponsable').val();
            var nom = $('#txtNombre').val();                        
            var est = $('#cboEstado').val();
            
            var row = $('#jqxgrid1').jqxGrid('getrows');
            var arr = [];            
            ...
            if (lider==='0' || resp.trim()==='' || nom.trim() === '' || est === '0') {
                $.msgBox({
                    title:"Atenci&#243;n",
                ...
                data.append('id',id);                       
                data.append('est', est)
                data.append('trab',JSON.stringify(arr));                
                ...
                cache:false,
                data: data,
                url:base_url+"epp/regProyecto/create",
                xhr: function() {
        ...
        function loadTrab(idProy){
            var source =
            {                               
                url: base_url+"epp/trabajador/getListPro?idPro="+idProy,                
                datatype: "json",
                root: 'data',
            ...
            var pagerrenderer = function () {
                ...
                rightButton.click(function () {
                    $("#jqxgrid1").jqxGrid('gotonextpage');
                });
                leftButton.click(function () {
                    $("#jqxgrid1").jqxGrid('gotoprevpage');
                });
                return element;
            }
            $("#jqxgrid1").on('pagechanged', function () {
                var datainfo = $("#jqxgrid1").jqxGrid('getdatainformation');
                var paginginfo = datainfo.paginginformation;
                self.label.text(1 + paginginfo.pagenum * paginginfo.pagesize + "-" + Math.min(datainfo.rowscount, (paginginfo.pagenum + 1) * paginginfo.pagesize) + ' de ' + datainfo.rowscount);
            });
		...
		height: "300px",                
                ...
                columns: [
                  ...
                  { text: '', dataField: 'checked',  width: '5%', cellsalign: 'center', align: 'center', columntype: 'checkbox'},
                  { text: 'Trabajador', dataField: 'nombre', width: '45%' },
                  { text: 'apellidos', dataField: 'apellidos', width: '50%' }
                ]
            });         
        }
