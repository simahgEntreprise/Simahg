...
<div class="modal fade" id="myModal" tabindex="-1" role="dialog" aria-labelledby="myModalLabel">
	...
	<form role="form" id="frmItem" enctype="multipart/form-data">
            <div class="form-group">
                <label>Login</label>
                <input class="form-control" id="login" style="width:50%" size="50" maxlength="50">
            </div>
            <div class="row">
            <div class="col-md-6">                    
                <div class="form-group">                
                    <label>Constraseña</label>
                    <input class="form-control" id="pass"  type="password" size="50" maxlength="50">
                    <input class="form-control" id="pass2" size="50" maxlength="50">
                </div>
            </div>
            <div class="col-md-6">                    
                <div class="form-group">                
                    <br>
                    <button type="button" class="btn btn-primary" id="btnShow" onclick="showPwd();">Mostrar</button>
                </div>
            </div>
            </div>
            <div class="row">
                <div class="col-md-6">                    
                    <div class="form-group">
                    <label>Nombre</label>
                    <input class="form-control" id="txtnombre" size="30" maxlength="30">
                    </div>
                </div>
                <div class="col-md-6">
                    <div class="form-group">
                    <label>apellidos</label>
                    <input class="form-control" id="txtapellido" size="30" maxlength="30">
                    </div>
                </div>
            </div> 
	...
	<input type="hidden" id="txtVer" size="1" maxlength="1">
    ...
    $(document).ready(function() {               
        ...
        var source =
            {                               
                url: base_url+"configuracion/usuario/getList", 
                datatype: "json",
                root: 'data',
                datafields: [
                    { name: 'id' },
    ...
    $('#idItem').val(id);
        $.ajax({
            ...
            success : function(text){
                ...
                $('#login').val(datos.usuario);
                $('#pass').val(datos.pasword);
                $('#txtnombre').val(datos.nombre);
                $('#txtapellido').val(datos.apellidos);
                $('#cboPerfil').val(datos.perfil.id);
                $('#cboCargo').val(datos.cargo.id);
    ...
    function deleteItem(id){
        $.ajax({
            ...
            datatype:'json',
            data: "idItem=" + id,
            url:base_url+"configuracion/usuario/delete",
            success : function(text){
                var data = $.parseJSON(text);            
                $.msgBox({
                        title:"",
        ...
        function create(){
            ...
            if (login.trim()==='' || pass.trim()==='' || nom.trim()==='' || ape.trim()==='' 
                 ||perfil ==='' || cargo==='' || estado==='' ) {
                $.msgBox({
                    title:"Atenci&#243;n",
                ...
                cache:false,
                data: data,
                url:base_url+"configuracion/usuario/create",
                success : function(text){
                    var data = $.parseJSON(text);                                        
                    if (data.success){
                        $('#btnCerrar').click();
        ...
        function showPwd(){
            if ($('#txtVer').val() ==='N' || $("#txtVer").val()===null){
                $('#txtVer').val('S');
                $("#pass2").show();                        
                $("#pass").hide();                                                        
                $("#btnShow").html('Ocultar');
            }else{
                $("#pass2").hide();                        
                $("#pass").show();                        
                $('#txtVer').val('N');
                $("#btnShow").html('Mostrar');
            }
            
            
            var data = new FormData();                
                data.append('tip','S');
                data.append('valor',$('#pass').val());
            ...
            $.ajax({
                ...
                cache:false,
                data: data,
                url:base_url+"configuracion/usuario/viewPass",
                success : function(text){
                    var data = $.parseJSON(text);                                        
                    if (data.success){                   
                        $("#pass2").val(data.msg);                        
